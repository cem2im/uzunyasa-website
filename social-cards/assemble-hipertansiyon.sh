...
set -e
BASE="$(dirname "$0")"
BG="$BASE/hipertansiyon-backgrounds"
OV="$BASE/hipertansiyon-slides"
TMP="/tmp/hipertansiyon-reel"
mkdir -p "$TMP"
...
echo "🎬 Compositing Hipertansiyon text overlays on background videos..."
...
echo ""
echo "🔗 Concatenating Hipertansiyon scenes with crossfade transitions..."
...
OUTPUT="$BASE/hipertansiyon-reel-final.mp4"
...
echo ""
echo "🎬 Hipertansiyon reel assembled!"
...
# Check if over 16MB, re-encode if needed
if [ $(echo "$FINAL_SIZE_MB > 16" | bc) -eq 1 ]; then
  echo ""
  echo "⚠️ File over 16MB — re-encoding with 2-pass..."
  REENCODED="$BASE/hipertansiyon-reel-final-small.mp4"
  ffmpeg -y -i "$OUTPUT" -c:v libx264 -b:v 4500k -pass 1 -f null /dev/null 2>/dev/null
  ffmpeg -y -i "$OUTPUT" -c:v libx264 -b:v 4500k -pass 2 "$REENCODED" 2>/dev/null
  rm -f ffmpeg2pass-0.log ffmpeg2pass-0.log.mbtree
  mv "$REENCODED" "$OUTPUT"
  FINAL_SIZE=$(stat -c%s "$OUTPUT")
  FINAL_SIZE_MB=$(echo "scale=1; $FINAL_SIZE / 1048576" | bc)
  echo "  Re-encoded size: ${FINAL_SIZE_MB}MB"
fi

echo ""
echo "📁 Copying to media folder..."
mkdir -p /home/clawdbot/.openclaw/media
cp "$OUTPUT" /home/clawdbot/.openclaw/media/hipertansiyon-reel.mp4
echo "  ✅ Copied to: /home/clawdbot/.openclaw/media/hipertansiyon-reel.mp4"
echo ""
echo "🎉 Hipertansiyon Instagram Reel production complete!"
echo "  Path: $OUTPUT"
